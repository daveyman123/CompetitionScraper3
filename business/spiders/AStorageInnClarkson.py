...
class AStorageInnLemay(scrapy.Spider):
	...
	def parse(self, response):
		...
		#url = "http://www.python.org"
                url='https://www.storageinns.net/self-storage/mo/ellisville/ruck-road/unit-sizes-prices#/units?category=all'
		...
		#driver2 = webdriver.Firefox()
                ...
                outside_units = ["5 x 10", "10 x 15","10 x 10","10 x 20","10 x 30","10 x 25"]
                    ...
                    if "Outside" in (typesTagz[n]).get_text():
                        if (sizeTagz[n]).get_text() in outside_units:
                        	if (sizeTagz[n]).get_text() not in size and Special != "":
                        	    ...
                        	    yield {"special": Special, "rate": re.findall('\d+\.\d+',(rateTagz[n]).get_text()),'size': ((sizeTagz[n]).get_text()),
"types": "Outside"        		}
		    if "Inside" in (typesTagz[n]).get_text():
			 if (sizeTagz[n]).get_text() in inside_units:
                                if (sizeTagz[n]).get_text() not in size2 and Special != "":
                                    size2.append((sizeTagz[n]).get_text())
                                    yield {"special": Special, "rate": re.findall('\d+\.\d+',(rateTagz[n]).get_text()),'size': ((sizeTagz[n]).get_text()),
"types": "Inside"                      }
