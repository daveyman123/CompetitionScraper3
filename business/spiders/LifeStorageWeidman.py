...
class MySpider(scrapy.Spider):
        name = "LifeStorageWeidman"

        
	
	allowed_domains = ['https://www.lifestorage.com']
	start_urls = ['https://www.lifestorage.com/storage-units/missouri/saint-louis/63021/307-in-ballwin/?size=5x5']
	...
	def parse(self, response):
                ...
                def replacenth(s, sub, repl, nth):
                    ...
                    return s
                url='https://www.lifestorage.com/storage-units/missouri/saint-louis/63021/307-in-ballwin/?size=5x5'
...
#                time.sleep(3)
		...
		rateTagz = soup.findAll('div',{"class":"priceBox"})
                ...
                #specialTagz = soup.select('div.priceBox nearbyPriceBox.specialsMessage')
		
		typesTagz = soup.findAll('ul',{"class":"features"},)
		s = soup.findAll('div',{"priceBox"})
		...
		for n in range(len(sizeTagz)):
                        ...
                        Special = s[n]
			SpecialMessage = Special.find('div',{"class":"specialsMessage"})
			if SpecialMessage != None:
				spec = SpecialMessage.get_text()
				spec = spec.replace("\n", "")
			else:
				spec = "None"
			if s == "":
				s = "Incomplete"
                        ...
                        if "Outdoor" in (typesTagz[n]).get_text() and "Indoor" not in (typesTagz[n]).get_text():
                                #print "Outdoor hit"
